...
def get_positions():
    ...
    # Grid positions from -2 to +2 (inclusive)
    grid_positions = list(product(range(-2, 3), repeat=3))
...
def finite_difference_approximations():
    """Define finite difference approximations for derivatives."""
    # Grid spacing is 1

    def dx_3point(i, j, k):
        return (-DATA(i - 1, j, k) + DATA(i + 1, j, k)) / 2

    def dy_3point(i, j, k):
        return (-DATA(i, j - 1, k) + DATA(i, j + 1, k)) / 2

    def dz_3point(i, j, k):
        return (-DATA(i, j, k - 1) + DATA(i, j, k + 1)) / 2

    def dxy_3point(i, j, k):
        return (
            DATA(i + 1, j + 1, k)
            - DATA(i - 1, j + 1, k)
            - DATA(i + 1, j - 1, k)
    ...
    return dx_3point, dy_3point, dz_3point, dxy_3point, dxz_3point, dyz_3point, dxyz_3point
...
def build_derivative_matching_equations(
    ...
    for i, j, k in fn_positions:
        eq_dx = sp.Eq(dpdx.subs({x_var: i, y_var: j, z_var: k}), dx_3point(i, j, k))
        eq_dy = sp.Eq(dpdy.subs({x_var: i, y_var: j, z_var: k}), dy_3point(i, j, k))
        eq_dz = sp.Eq(dpdz.subs({x_var: i, y_var: j, z_var: k}), dz_3point(i, j, k))
        eq_dxy = sp.Eq(dpdxdy.subs({x_var: i, y_var: j, z_var: k}), dxy_3point(i, j, k))
        eq_dxz = sp.Eq(dpdxdz.subs({x_var: i, y_var: j, z_var: k}), dxz_3point(i, j, k))
        eq_dyz = sp.Eq(dpdydz.subs({x_var: i, y_var: j, z_var: k}), dyz_3point(i, j, k))
        eq_dxyz = sp.Eq(
            dpdxdydz.subs({x_var: i, y_var: j, z_var: k}), dxyz_3point(i, j, k)
        )
...
if __name__ == "__main__":
    ...
    # List of derivatives to process
    derivative_list = [
        ("dx", derivatives[0]),
        ("dy", derivatives[1]),
        ("dz", derivatives[2]),
